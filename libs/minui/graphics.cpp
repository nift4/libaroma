...
GRSurface *gr_create(size_t w, size_t h, size_t rb, size_t pb){
	printf("Creating surface with w=%d, h=%d, rb=%d and pb=%d\n", w, h, rb, pb);
	std::unique_ptr<GRSurface> surface = GRSurface::Create(w, h, rb, pb);
	GRSurface *pointer=surface.get();
	if (pointer==nullptr) printf("Failed to create surface! Returning NULL...\n");
	return pointer;
}

GRSurface *gr_init() {
  printf("gr_init: minui (probably v11.0) starting up\n");
...
*/
  printf("gr_init: setting pixelformat\n");
  pixel_format = PixelFormat::BGRA;

  printf("gr_init: setting backend (DRM)\n");
  auto backend = std::unique_ptr<MinuiBackend>{ std::make_unique<MinuiBackendDrm>() };
  printf("gr_init: starting backend (DRM)\n");
  gr_draw = backend->Init();

  /*
   * Here it was the fbdev handler in case drm of init failing,
   * but as we use minui only for DRM support we don't need it.
   */

  if (!gr_draw) {
	printf("gr_init: backend could not be started\n");
    return NULL;
  }

  printf("gr_init: releasing backend\n");

  gr_backend = backend.release();

  printf("gr_init: setting overscan percent (0)\n");

  int overscan_percent = 0;//android::base::GetIntProperty("ro.minui.overscan_percent", 0);
  overscan_offset_x = gr_draw->width * overscan_percent / 100;
  overscan_offset_y = gr_draw->height * overscan_percent / 100;

  printf("gr_init: flipping screen buffers twice\n");
  gr_flip();
  gr_flip();
  if (!gr_draw) {
  ...
  if (0){ //handle framebuffer rotation pls
  /*

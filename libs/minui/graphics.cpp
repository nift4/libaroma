...
GRSurface *gr_init() {
  ...
  overscan_offset_x = 0;//gr_draw->width * overscan_percent / 100;
  overscan_offset_y = 0;//gr_draw->height * overscan_percent / 100;
...
GRSurface *gr_create(int w, int h, int row_bytes, int pixel_size){
	auto surface = GRSurface::Create(w, h, row_bytes, pixel_size);

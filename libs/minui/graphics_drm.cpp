...
GRSurface* MinuiBackendDrm::Init() {
  printf("MinuiBackendDrm::Init: starting up DRM engine\n");
  drmModeRes* res = nullptr;
  drm_fd = -1;

  printf("MinuiBackendDrm::Init: looking for DRM devices\n");
  /* Consider DRM devices in order. */
  for (int i = 0; i < DRM_MAX_MINOR; i++) {
    printf("MinuiBackendDrm::Init: DRM_DEV_NAME = %s, DRM_DIR_NAME = %s, i = %d\n", DRM_DEV_NAME, DRM_DIR_NAME, i);
    auto dev_name = android::base::StringPrintf("/dev/dri/card0");
    android::base::unique_fd fd(open(dev_name.c_str(), O_RDWR | O_CLOEXEC));
    printf("dev_name: %s\n", dev_name.c_str());
    if (fd == -1) {
		printf("MinuiBackendDrm::Init: could not open unique pipe to DRM card\n");
		continue;
	}

    /* We need dumb buffers. */
    if (uint64_t cap = 0; drmGetCap(fd.get(), DRM_CAP_DUMB_BUFFER, &cap) != 0 || cap == 0) {
	  printf("MinuiBackendDrm::Init: DRM reported that this device doesn't support dumb framebuffers\n");
      continue;
    }

    res = drmModeGetResources(fd.get());
    if (!res) {
	  printf("MinuiBackendDrm::Init: failed to get device's resources\n");
      continue;
    }
	...
	printf("MinuiBackendDrm::Init: something failed, fix your code pls\n");
  ...
  printf("MinuiBackendDrm::Init: looking for main monitor/display connector\n");
  ...
  printf("MinuiBackendDrm::Init: looking for main monitor/display crtc\n");
  ...
  printf("MinuiBackendDrm::Init: disabling other displays than main\n");
  DisableNonMainCrtcs(drm_fd, res, main_monitor_crtc);

  main_monitor_crtc->mode = main_monitor_connector->modes[selected_mode];

  printf("MinuiBackendDrm::Init: getting width/height \n");

  int width = main_monitor_crtc->mode.hdisplay;
  int height = main_monitor_crtc->mode.vdisplay;

  printf("MinuiBackendDrm::Init: width: %d, height: %d\n", width, height);

  drmModeFreeResources(res);

  printf("MinuiBackendDrm::Init: creating surfaces\n");
  ...
  if (!GRSurfaceDrms[0] || !GRSurfaceDrms[1]) {
	printf("graphics_drm.cpp: Surfaces weren't created\n");
    return nullptr;
  }

  current_buffer = 0;

  printf("MinuiBackendDrm::Init: enabling main monitor crtc\n");
  ...
  if (!DrmEnableCrtc(drm_fd, main_monitor_crtc, GRSurfaceDrms[1])) {
	printf("graphics_drm.cpp: Crtc enabling failed\n");
    return nullptr;
  }

  printf("MinuiBackendDrm::Init: all done, come back later! (if you really do so the program will crash, please don't take this seriously)\n");

  return GRSurfaceDrms[0].get();
}

static void page_flip_complete( int fd,

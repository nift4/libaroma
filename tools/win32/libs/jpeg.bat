@echo off 
REM Thanks https://stackoverflow.com/a/7006016 for the explaination
if not "x%LIBAROMA_CFLAGS:ARM_HAVE_NEON=%"=="x%LIBAROMA_CFLAGS%" (
	mkdir ..\obj\jpeg
	cd ..\obj\jpeg

	echo ARM NEON enabled, compiling jpeg
	%LIBAROMA_GCC% -c ^
	...
	cd ..\..\libs
		...
		echo ARM NEON disabled, not compiling JPEG.
)
